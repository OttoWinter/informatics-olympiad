...
void update(Segment *seg, index_t seg_from, index_t seg_to, index_t want, index_t value) {
  if (want < seg_from || seg_to < want)
    return;
  if (seg_from == seg_to) {
    seg->best_prefix = seg->best_suffix = seg->best = seg->sum = value;
    return;
  }

  index_t seg_mid = (seg_from + seg_to) / 2;
  Segment *left_child = seg->left_child;
  update(left_child, seg_from, seg_mid, want, value);
  Segment *right_child = seg->right_child;
  update(right_child, seg_mid + 1, seg_to, want, value);
